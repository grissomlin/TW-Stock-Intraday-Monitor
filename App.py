...
st.markdown("""
    <div class="welcome-header">
        ...
        <p style="font-size: 1rem; opacity: 0.8;">📅 分析日期：{today} | 🕐 最後更新：{current_time}</p>
    </div>
""".format(
    today=today,
...
if supabase:
    ...
    if not df_limit_ups.empty:
        # 顯示前10檔漲停股票
        display_cols = ['stock_name', 'symbol', 'sector', 'return_rate', 'price', 'consecutive_days']
        display_df = df_limit_ups[display_cols].head(10).copy()
        display_df.columns = ['股票名稱', '代碼', '產業', '漲幅', '價格', '連板天數']
        ...
        with col_s3:
            avg_days = df_limit_ups['consecutive_days'].mean() if 'consecutive_days' in df_limit_ups.columns else 1
