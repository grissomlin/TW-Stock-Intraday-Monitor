...
def run_monitor():
    ...
    stock_dict = {stock['symbol']: stock for stock in stocks} # 快速查找 name/sector 等

    # 批量下載設定
    batch_size = 150 # 可調整，100~200 都安全
    ...
    # 儲存市場總結（無條件執行，即使無漲停也要記錄）
    if supabase:
        try:
            today_str = datetime.now().strftime("%Y-%m-%d")
           
            # 即使沒有漲停，也要統計產業分佈（空字典即可）
            sector_counts = {}
            if limit_up_stocks:
                for stock in limit_up_stocks:
                    sector = stock['sector']
            ...
            # 儲存到 daily_market_summary
            safe_data = {
                "analysis_date": today_str,
                "stock_count": total_stocks,          # 對應你的 stock_count 欄位
            ...
            log("✅ 已儲存市場總結到 daily_market_summary（即使無漲停）")
